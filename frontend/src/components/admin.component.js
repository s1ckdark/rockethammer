    ...
    switch(type) {
      case 'update':
        ...
        await axios.post(process.env.REACT_APP_API+"/user/get",{
          ...
          if(res.data.password !== this.state.user.data[index].password) ele.push("비밀번호")
          if(res.data.name !== this.state.user.data[index].name) ele.push("이름")
          if(res.data.dept !== this.state.user.data[index].dept) ele.push("소속")
          if(res.data.group !== this.state.user.data[index].group) ele.push("그룹")
        })
        console.log(ele);
        historytype = ele.join(", ")+" 항목을 "+conType;
        axios.post(process.env.REACT_APP_API+"/user/update/"+userid,
          this.state.user.data[index]).then(res => this.writeHistory(e, historytype , userid, index))
      } else {
        alert("취소됩니다");
      } 
        break;
  ...
  render() {
    ...
    <div className={this.state.toggle === 2  ? "history active":"history"}>
           ...
           <thead>
              ...
              <th className="text-center my-3 p-3 col-md-3">수정대상</th>
              <th className="text-center my-3 p-3 col-md-5">수정내역</th>
        ...
        {this.state.history.totalcnt > 0 ? this.state.history.currentTableData.map((log, index)=>{
          ...
          <tr>
            ...
            <td className="text-center my-3 p-3 col-md-3">{log.userid}</td>
            <td className="text-center my-3 p-3 col-md-5">{log.mod_item}</td>
